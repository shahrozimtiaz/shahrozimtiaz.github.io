import java.io.*;
import java.util.*;
public class TestMovieDB1{

    public static void main(String[] args)throws IOException{
        //Read in movies to query!
        FileInputStream fis = new FileInputStream("./allMovieData.txt");
        ...
        for(int i=0; i<amount; i++){
            int moviesFound=0;
            fis = new FileInputStream("./procMovieData.txt");
            ...
            fis = new FileInputStream("./procMovieData.txt");
            reader = new Scanner(fis);
            MovieDBImp1 mdbi = new MovieDBImp1();
            st = System.nanoTime();
            while(reader.hasNext()){ //reading in
                String[] parts = reader.nextLine().split(",");
                Movie m = new Movie(parts[0],parts[1],parts[2],Integer.parseInt(parts[3]));
