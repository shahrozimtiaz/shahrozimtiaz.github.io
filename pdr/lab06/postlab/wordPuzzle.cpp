...
int main(int argc, char* argv[]) {
	...
	while(file >> temp){
		if(temp.length()>2){
			hash_table->put(temp);
            if(temp.length()>largestWord)
                largestWord = temp.length(); //used for optimization
		}
	}
    ...
    for(int y = 0;y<=rows;y++){
        for(int x = 0;x<=cols;x++){
            for(int d=0;d<=7;d++){
                for(int l=3;l<=largestWord;l++){//optimized
                    ...
                    if(hash_table->contains(possibleWord)){
                        ...
                        ostringstream xv;
                        xv << x;
                        word+=" (" + yv.str() + ", " + xv.str() + "): " + possibleWord;
